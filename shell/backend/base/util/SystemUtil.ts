...
export class SystemUtil extends BaseClass {
    ...
    * */
    ...
    public formatArgv(argv: string[]) {
        ...
        startParam.env = startParam.env ? startParam.env : EnvStr.Dev
        return startParam;
    }
    ...
    public InitLog4js(level: string = "debug") {
        ...
        configure({
            appenders: {
                fileout: {
                    type: "file",
                    filename: App.configManager.getServer().userLogDir + `/${serverId}.log`,
        ...
        const logger = getLogger(App.configManager.getServer().loggerName || "console");
