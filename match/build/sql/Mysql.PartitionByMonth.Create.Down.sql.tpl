...
string getPartitionDate()
{
	time_t now = time(NULL);
	tm* tm = localtime(&now);
	time_t nowNext = time(NULL) + (32 - tm->tm_mday) * 24 * 60 * 60;
	char timeBuffer[100];
	strftime(timeBuffer, 100, "%Y-%m-01", localtime(&nowNext));
	return timeBuffer;
}

string getPartitionMonth()
{
	time_t now = time(NULL);
	char timeBuffer[100];
	strftime(timeBuffer, 100, "%Y%m", localtime(&now));
	return timeBuffer;
}
...
ALTER TABLE !!@pre!!!!@name!! RENAME TO !!@pre!!Temp_!!@name!!;
	!!endif!!
	!!next!!
!!leave!!
!!enter XTP!!
...
(
	...
	!!endif!!
		!!if valid_name("DbKeys")!!
			...
			!!if !strcmp(@CreateTime,"yes")!!
	,KEY mykey_!!command display_string(skeyname)!! (!!command display_string(vct[i].c_str())!!,CreateTime)
			!!else!!
...
!!let partitionDate=getPartitionDate().c_str()!!
!!let partitionMonth=getPartitionMonth().c_str()!!
)ENGINE=InnoDB COMMENT='!!@comment!!'
PARTITION BY RANGE COLUMNS(CreateTime)
(PARTITION p!!@partitionMonth!! VALUES LESS THAN ('!!@partitionDate!!') ENGINE = InnoDB);
		!!else!!
	)ENGINE=InnoDB COMMENT='!!@comment!!';
		!!endif!!
	!!endif!!
