...
typedef struct tagMyDoubleType
{
	double value;
	int precision;
} MyDoubleType;
...
protected:
	...
	void GetJsonValue(MyDoubleType& value, char* szValue, int len);
	...
	void GetQueryValue(MyDoubleType& value, char* szValue, int len);
	...
	void AppendTriggerPrice(const char* key, MegaOrderInsertStruct* pOrderInsert, char* target, int pricePrecision) {}
	void AppendTriggerPrice(const char* key, MegaTriggerOrderInsertStruct* pOrderInsert, char* target, int pricePrecision)
	{
		MyDoubleType value = { pOrderInsert->TriggerPrice, pricePrecision };
		AppendQueryData(target, key, value);
	}
	...
	{
		MyDoubleType value = { pOrderInsert->SLTriggerPrice, 1 };
		AppendQueryData(target, key, value);
	}
...
int CLink::SendOrder(T1* pOrderInsert, const char* type, string& errMsg, int nRequestNo)
{
	...
	vector< string > vct;
	Txtsplit(pOrderInsert->Remark, ",", vct);
	for (vector< string >::iterator it = vct.begin(); it != vct.end(); it++)
	{
		int len = 2;
		if (0 == strncmp(it->c_str(), "v:", len))
			volumePrecision = atoi(it->c_str() + len);
		else if (0 == strncmp(it->c_str(), "p:", len))
			pricePrecision = atoi(it->c_str() + len);
	}
	...
	MyDoubleType quantity = { pOrderInsert->Volume, volumePrecision };
	AppendQueryData(buff, "quantity", quantity);
	...
	{
		MyDoubleType value = { pOrderInsert->Price, pricePrecision };
		AppendQueryData(buff, "price", value);
	...
	if (0 == strcmp(type, "STOP_MARKET") 
		...
		|| 0 == strcmp(type, "TAKE_PROFIT"))
	{
		AppendTriggerPrice("stopPrice", pOrderInsert, buff, pricePrecision);
	}

	if (0 == strcmp(type, "Trailing_Stop_Market"))
	{
		AppendTriggerPrice("activationPrice", pOrderInsert, buff, pricePrecision);
		AppendSLTriggerPrice("callbackRate", pOrderInsert, buff);
	}

	if (pOrderInsert->OffsetFlag == Mega_OffsetFlag_Close)
	{
		AppendQueryData(buff, "reduceOnly", "true");
	}
	else if (pOrderInsert->OffsetFlag == Mega_OffsetFlag_CloseAll)
	{
		AppendQueryData(buff, "closePosition", "true");
	}
	...
	char resp[1024] = {};
	bool res = m_httpClient.Post("/fapi/v1/order", buff, m_extraHeader.c_str(), resp, sizeof(resp) - 1);
...
int CLink::OrderAction(T1* pOrderAction, string& errMsg, int nRequestNo)
{
	...
	if (strlen(pOrderAction->OrderID) > 0)
		AppendQueryData(buff, "orderId", pOrderAction->OrderID);
	else
		AppendQueryData(buff, "origClientOrderId", pOrderAction->OrderLocalID);
	...
	AppendQueryData(buff, "signature", signature.c_str());
	char resp[1024] = {};
	bool res = m_httpClient.Delete("/fapi/v1/order", buff, m_extraHeader.c_str(), resp, sizeof(resp) - 1);
