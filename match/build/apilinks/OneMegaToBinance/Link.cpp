...
{
	/**
	{"e":"ORDER_TRADE_UPDATE","T":1683730506462,"E":1683730506467,"o":{"s":"BTCUSDT","c":"web_mTh6ig1KhUrmyrXsyUWI"
	,"S":"BUY","o":"MARKET","f":"GTC","q":"1","p":"0","ap":"0","sp":"0","x":"NEW","X":"NEW","i":3343348825
	,"l":"0","z":"0","L":"0","n":"0","N":"USDT","T":1683730506462,"t":0,"b":"0","a":"0","m":false,"R":false
	,"wt":"CONTRACT_PRICE","ot":"MARKET","ps":"BOTH","cp":false,"rp":"0","pP":false,"si":0,"ss":0}}

	{"e":"ACCOUNT_UPDATE","T":1683731115472,"E":1683731115474,"a":{"B":[{"a":"USDT","wb":"32677.20174619"
	,"cw":"32677.20174619","bc":"0"}],"P":[{"s":"BTCUSDT","pa":"-4","ep":"29171.42805","cr":"11752.92078960"
	,"up":"4181.69029524","mt":"cross","iw":"0","ps":"BOTH","ma":"USDT"}],"m":"ORDER"}}
...
//string strData((char*)realData, realDatalen);
	...
	try
	{
		...
		if (0 == strcmp(dataType, "ORDER_TRADE_UPDATE"))
		{
			PRINT_TO_STD("ORDER_TRADE_UPDATE:%s", strData.c_str());
			m_pLink->OnOrderUpdate(root["o"]);
		}
...
void CLink::Connect()
{
	...
	if (m_bHasTrade)
	{
		...
		if (m_userLogin.UserID[0] != 0 || m_userLogin.Password[0] != 0)
		{
...
int CLink::SendQryInstrument(MegaQryInstrumentStruct* pQryInstrument, int nRequestNo)
{
	...
	do
	{
		if (!m_httpClient.Get("/fapi/v1/exchangeInfo", NULL, m_extraHeader.c_str(), resp, resp_len - 1))
			break;
...
void CLink::OnPoll()
{
	...
	char resp[1024] = {};
	bool res = m_httpClient.Put("/fapi/v1/listenKey", NULL, m_extraHeader.c_str(), resp, sizeof(resp) - 1);
	if (res)
	{
...
void CLink::OnQryInst(Json::Value& Value, bool bIsLast)
{
	...
	snprintf(inst.Remark, sizeof(inst.Remark) - 1, "p:%d,v:%d", Value["pricePrecision"].asInt(), Value["quantityPrecision"].asInt());
...
void CLink::GetJsonValue(MyDoubleType& value, char* szValue, int len)
{
	snprintf(szValue, len - 1, "\"%.*lf\"", value.precision, value.value);
}
...
void CLink::GetQueryValue(MyDoubleType& value, char* szValue, int len)
{
	snprintf(szValue, len - 1, "%.*lf", value.precision, value.value);
}
...
{
	char resp[1024] = {};
	bool res = m_httpClient.Post("/fapi/v1/listenKey", NULL, m_extraHeader.c_str(), resp, sizeof(resp) - 1);
	if (!res)
	{
