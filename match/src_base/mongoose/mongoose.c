...
struct mg_connection *mg_connect_http_opt(
  ...
  mg_printf(nc, "%s %.*s HTTP/1.1\r\n%s\r\nHost: %.*s\r\nContent-Length: %" SIZE_T_FMT
                "\r\n%.*s\r\n%s",
            method, (int) path.len, path.p, extra_headers,
            (int) (path.p - host.p), host.p, strlen(post_data), (int) auth.len,
            (auth.buf == NULL ? "" : auth.buf), post_data);
...
struct mg_connection *mg_connect_http(
    struct mg_mgr *mgr, MG_CB(mg_event_handler_t ev_handler, void *user_data),
    const char *url, const char* method, const char *extra_headers, const char *post_data) {
  struct mg_connect_opts opts;
  memset(&opts, 0, sizeof(opts));
  return mg_connect_http_opt(mgr, MG_CB(ev_handler, user_data), opts, url, method,
