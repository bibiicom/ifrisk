...
static void q_copyin(struct mg_queue* q, const uint8_t *buf, size_t len,
                     size_t head) {
...
static void q_copyout(struct mg_queue* q, uint8_t *buf, size_t len, size_t tail) {
  size_t left = q->len - tail;
  memcpy(buf, &q->buf[tail], left < len ? left : len);
  if (left < len) memcpy(&buf[left], q->buf, len - left);
}

static bool q_write(struct mg_queue* q, const void *buf, size_t len) {
  bool success = false;
...
#ifdef MIP_QPROFILE
static inline size_t q_space(struct mg_queue* q) {
  return q->tail > q->head ? q->tail - q->head : q->tail + (q->len - q->head);
}
#endif

static inline size_t q_avail(struct mg_queue* q) {
  size_t n = 0;
  if (q->tail != q->head) q_copyout(q, (uint8_t *) &n, sizeof(n), q->tail);
  return n;
}

static size_t q_read(struct mg_queue* q, void *buf) {
  size_t n = q_avail(q);
  if (n > 0) {
